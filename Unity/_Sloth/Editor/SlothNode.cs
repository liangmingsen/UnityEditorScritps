...
public partial class Sloth : Editor
{
    ...
    /// 选中物体的面数
    /// </summary>
    [MenuItem("Sloth/Node/SelectNodeMeshTriCount")]
    static void SelectNodeMeshTriCount()
    {
        int count = 0;
        GameObject[] objs = Selection.gameObjects;
        foreach (GameObject obj in objs)
        {
            count += NodeUtil.GetMeshTriCount(obj);
        }
        Debug.Log("tris count:" + count);
    }
    ...
    //选中的物体的节点个数，包括所有的子节点
    [MenuItem("Sloth/Node/Select Nodes Count Recursion")]
    static void CheckSelectNodesCountRecursion()
    {
        NodeUtil.CheckSelectNodesCountRecursion();
    }
