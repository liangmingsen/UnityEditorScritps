...
public class NodeUtil
{
    ...
    private static int DoGetMeshTriCount(Transform ts)
    {
        int count = 0;
        if (ts != null)
        {
            MeshFilter mf = ts.GetComponent<MeshFilter>();
            if (mf != null)
            {
            ...
            {
                count += DoGetMeshTriCount(ts.GetChild(i));
            }
        }
        return count;
    }

    public static int GetMeshTriCount(GameObject obj)
    {
        int count = 0;
        if (obj != null)
        {
            count = DoGetMeshTriCount(obj.transform);
        }
        return count;
    }
    ...
    private static int GetAllNodesCount(Transform ts)
    {
        int count = 1;
        for (int i = 0; i < ts.childCount; i++)
        {
            count += GetAllNodesCount(ts.GetChild(i));
        }
        return count;
    }

    public static void CheckSelectNodesCountRecursion()
    {
        int count = 0;
        Transform[] tfs = Selection.transforms;
        for (int i = 0; i < tfs.Length; i++)
        {
            count += GetAllNodesCount(tfs[i]);
        }
        Debug.Log("obj count recursion: " + count);
    }
    ...
    public static void SamePosZnodes()
    {
        Transform[] tfs = Selection.transforms;
        foreach (Transform t in tfs)
        {
